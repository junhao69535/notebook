...
"""
    ...
    # 默认是www子域名，如果需要使用kb域名，则MapAdptor的match方法需要指定subdomain。
    ...
    recommended.
    # endpoint可以是任何对象，推荐为str。endpoint用于映射视图函数处理请求。

    Now it's possible to create a URL adapter for one of the subdomains and
    build URLs:

    >>> c = m.bind('example.com')
    >>> c.build("kb/browse", dict(id=42))  # 根据endpoint重构url
    'http://kb.example.com/browse/42/'
    >>> c.build("kb/browse", dict())
    'http://kb.example.com/browse/'
    >>> c.build("kb/browse", dict(id=42, page=3))
    'http://kb.example.com/browse/42/3'
    >>> c.build("static/about")  # 重构默认子域名的url不会显示绝对url，除非force_external为True
    '/about'
    ...
    >>> c = m.bind('example.com')
    >>> c.match("/")  # url映射endpoint，然后endpoint和捕捉的路由参数
    ('static/index', {})
    >>> c.match("/about")
    ('static/about', {})
    ...
    exception is raised.
    # 如果匹配成功，但请求方法不允许，抛出MethodNotAllowed异常。
...
class Rule(RuleFactory):
    ...
    `string`
        ...
        # 如果不提供converter，默认使用string converter

        URL rules that end with a slash are branch URLs, others are leaves.
        If you have `strict_slashes` enabled (which is the default), all
        branch URLs that are matched without a trailing slash will trigger a
        redirect to the same URL with the missing slash appended.
        # 那些url以"/"结尾的称为分支url，其他的称为叶子url。如果定义url rules为分支
        # url，但请求给出的是叶子url，那么会触发重定向（308），重定向到叶子url。除非
        # 关闭strict_slashes（默认是开启的）。
        # example
        # Rule("/home/", endpoint="home/index")
        # 如果请求的是/home,则会发起重定向到/home/
    ...
    `subdomain`
        ...
        # 如果不指定subdomain，默认使用map的default_subdomain，如果map也没有绑定
        # subdomain，关闭subdomain这项功能
    ...
    `methods`
        ...
        # 如果GET请求方法在methods里，自动加上HEAD请求方法。
    ...
    `strict_slashes`
        ...
        # 如果提供了，重载map的strict_slashes

    `build_only`
        Set this to True and the rule will never match but will create a URL
        that can be build. This is useful if you have resources on a subdomain
        or folder that are not handled by the WSGI application (like static data)
        # 如果True，则不能进行路由匹配，但能反向构建url。
    ...
    `redirect_to`
        ...
        rule syntax::
        # 如果提供一个可调用对象，那么必须提供adapter参数用于匹配，同时可以提供路由关键字。
        ...
        # 当提供了redirect_to且触发后会抛出RequestRedirect异常。
    ...
    `alias`
        ...
        # 如果设置了alias，这个alias可以指向其他路由endpoint，那么可以重定向到那个endpoint。

    `host`
        If provided and the URL map has host matching enabled this can be
        used to provide a match rule for the whole host.  This also means
        that the subdomain feature is disabled.
        # 如果提供了，则匹配路由时需要匹配host，且subdomain功能失效。
...
class Map(object):
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        feature.
        # 由于协议的限制，无法从environ获取当前subdomain和真实的server_name。如果不提供，
        # server_name为environ中的SERVER_NAME和SERVER_PORT结合且不提供subdomain功能。
        ...
        # 如果提供了environ和server_name，那么会自动计算subdomain
...
除了在初始化的时候添加路由规则，也可以动态添加路由规则，通过add方法，这个方法核心工作是`rule.bind(self)`，这个我们在分析Rule的bind方法已经分析过了，就是进行路由注册。
